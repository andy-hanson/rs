	...
	fn show<S : Shower>(self, s: &mut S) -> Result<(), S::Error> {
		match *self {
			EmitError::MissingBuiltin(method) => {
				...
				s.add("Missing builtin impl for method '")?.add(method)?.add("'.")?;
			}
			EmitError::WrongBuiltinArity { expected, actual } => {
				s.add("Expected method '")?.add(expected)?.add("' to have arity ")?.add(expected.arity())?.add(", but builtin impl has arity ")?.add(actual)?;
			}
		}
		Ok(())
	}
