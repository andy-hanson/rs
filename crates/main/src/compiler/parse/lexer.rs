...
pub struct Lexer<'arena, 'text> {
	pub arena: &'arena Arena,
	reader: Reader<'text>,
	indent: u32,
	dedenting: u32,
	quote_part_value: &'arena [u8],
	diagnostic: Option<Diagnostic>,
}
impl<'arena, 'text> Lexer<'arena, 'text> {
	pub fn new(arena: &'arena Arena, source: &'text [u8]) -> Self {
		Lexer {
			arena,
	...
	pub fn quote_part_value(&mut self) -> &'arena [u8] {
	...
	pub fn expr_from(&self, start: Pos, data: ExprData<'arena>) -> &'arena Expr<'arena> {
		self.arena <- Expr(self.loc_from(start), data)
	}

	pub fn direct_arr_builder<T : Sized + NoDrop>(&self) -> DirectArrBuilder<'arena, T> {
		self.arena.direct_arr_builder()
	}

	pub fn list_builder<T : Sized + NoDrop>(&self) -> ListBuilder<'arena, T> {
	...
	pub fn take_ty_name_slice(&mut self) -> Result<&'arena [u8]> {
