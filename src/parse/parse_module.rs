...
fn parse_head(l: &mut Lexer, start: Pos, kw: Token) -> Result<(Option<ast::Head>, Pos, MethodKw)> {
	match kw {
		...
		Token::Enum =>
			panic!(), // TODO
		_ =>
			Err(l.unexpected_token(start, kw, "'abstract', 'static', 'slots', or 'enum'"))
	}
}
...
fn parse_abstract_head(l: &mut Lexer, start: Pos) -> Result<ast::Head> {
	...
	loop {
		...
		abstract_methods.add(ast::AbstractMethod::of(
			l.loc_from(method_start), type_parameters, return_ty, name, self_effect, parameters));
...
fn parse_methods(l: &mut Lexer, mut start: Pos, mut next: MethodKw) -> Result<Arr<ast::Method>> {
	...
	loop {
		match next {
			...
			MethodKw::Eof =>
				break Ok(methods.finish()),
			MethodKw::Is =>
				break Err(l.unexpected_token(start, Token::Is, "'def' or 'fun'"))
		}
	}
}
	...
	let (self_effect, parameters): (Effect, Arr<ast::Parameter>) = if l.try_take_parenr() {
		...
		match parse_self_effect_or_ty(l)? {
			Either::Left(self_effect) => {
				let parameters = if l.try_take_parenr() { Arr::empty() } else { parse_parameters(l, None)? };
...
fn parse_supers(l: &mut Lexer, mut start: Pos, mut next: MethodKw) -> Result<(Arr<ast::Super>, Pos, MethodKw)> {
...
fn parse_slots(l: &mut Lexer, start: Pos) -> Result<ast::Head> {
	...
	loop {
		...
		let mutable = match next { SlotKw::Val => false, SlotKw::Var => true };
		l.take_space()?;
		let ty = parse_ty(l)?;
		l.take_space()?;
		let name = l.take_name()?;
