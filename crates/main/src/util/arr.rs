...
pub trait SliceOps<T>: Index<usize, Output = T> {
	...
	fn zip<U, V, I : Iterator<Item=U>, F: Fn(&T, U) -> V>(&self, other: I, f: F) -> Arr<V> {
		self.try_zip(other, f).unwrap()
	}

	fn try_zip<U, V, I : Iterator<Item=U>, F: Fn(&T, U) -> V>(&self, other: I, f: F) -> Result<Arr<V>, (usize, usize)> {
		let mut b = ArrBuilder::<V>::new(); //TODO:PERF pre-allocate my length
	...
	//TODO:NEATER
	/*fn zip_list<U, V, F: Fn(&T, &U) -> V>(&self, other: &List<U>, f: F) -> Arr<V> {
...
pub trait CloneSliceOps<T: Clone>: SliceOps<T> {
	...
	fn map_on_clones<U, F: Fn(T) -> U>(&self, f: F) -> Arr<U> {
		let mut b = ArrBuilder::new();
		for x in self.iter() {
			b.add(f(x.clone()))
		}
